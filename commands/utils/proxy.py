...
class Proxy(commands.Cog):
        ...
        **Sintaxis:** **``=proxy <tipo> <numero de proxys>``**
        ...
        try:
            ...
            if type.lower() not in ['http', 'https', 'socks5', 'socks4']:
                await ctx.reply(
                    "Ese tipo de proxy no es válido, prueba con una de las  siguientes: **http**, **socks4**, **socks5**")
                return
    ...
    async def on_error(self, ctx: commands.Context, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.reply(
                'Al comando le faltan argumentos! recuerda que el comando recibe como parametros el **tipo** ***(http, https, socks4, socks5)*** y el numero de proxies **(maximo:40)**')
